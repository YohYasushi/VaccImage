...
$arrayA[htmlspecialchars($row['vaccdate'])] = CountVial($row['sum(AdCount)'], $row['sum(CdCount)'], $r, $n);//接種日付とバイアル数の連想配列($arrayA)の作成
}
echo '<tr>';
echo '<td>', '合計' , '</td>';
echo '<td>', $sumAd , '</td>';
...
//上記配列の日付が逐一接種がある日かどうか判断し、使用したバイアル数と合わせた相関配列($arrayDateVial)を作る
$arrayDateVial = [];
for ($i = 0; $i <= $count; $i++){

 if(isset($arrayA[$arrayDates[$i]])){
...
//さらにこの配列の累積の配列($arraySumDateVial)を作る
...
foreach($stt as $row)  {  
    ...
    echo '</tr>';
    $arrayB[htmlspecialchars($row['shuseidate'])] = htmlspecialchars($row['shuseicount']);//日付とバイアル入荷数の連想配列($arrayB)の作成
...
<?php
//1日に複数回の入荷・払い出しイベントがあることを考慮し、新たなViewをSQL上で作成する
//それの累計から、日付配列に基づいた累積の配列を求める。
...
foreach($stt as $row)  {  
   ...
   $arrayB[htmlspecialchars($row['shuseidate'])] = htmlspecialchars($row['sum(shuseicount)']);//日付と各日付ごとのバイアル入荷数の連想配列( $arrayB)のに置換する(1日複数回の入荷・払い出しイベントを考慮し計算したもの)

}

$arrayDateShusei= [];

//上記配列( $arrayB)の日付が逐一、入荷・払い出しイベントがある日かどうか判断し、使用したバイアルと合わせた相関配列($arrayDateShusei)を作る
for ($i = 0; $i <= $count; $i++){
if(isset($arrayB[$arrayDates[$i]])){
...
//さらにこの配列の累積の配列( $arraySumDateShusei)を作る
//接種イベントがない日でも累積使用バイアル数を知ることができるようになった。
$arraySumDateShusei = [];
for ($i = 0; $i <= $count; $i++){
